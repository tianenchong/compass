...
describe.skip('hadron-build::release', function() {
  ...
  it('should symlink `Electron` to the app binary on OS X', function(done) {
    ...
    const bin = target.dest(`${target.productName}-darwin-arm64`, `${target.productName}.app`, 'Contents', 'MacOS', 'Electron');
  ...
  it('sets the correct CFBundleIdentifier', function() {
    ...
    const info = target.dest(`${target.productName}-darwin-arm64`, `${target.productName}.app`, 'Contents', 'Info.plist');
